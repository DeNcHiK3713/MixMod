...
{
    [HarmonyPatchCategory("Others_MoveEnemyCards")]
    [HarmonyPatch(typeof(ZoneHand), nameof(ZoneHand.GetCardPosition), [typeof(int), typeof(int)])]
    ...
    [HarmonyPatchCategory("Others_MoveEnemyCards")]
    [HarmonyPatch(typeof(ZoneHand), nameof(ZoneHand.GetCardRotation), [typeof(int), typeof(int)])]
            ...
            if (index > 0)
            {
                ...
                newInstructions.Insert(index++, new CodeInstruction(OpCodes.Newobj,
                        typeof(Vector3).GetConstructor(BindingFlags.Instance | BindingFlags.Public,
                            null, [typeof(float), typeof(float), typeof(float)], null)));
